# Phase 01 — Market Mechanics

**Goal:** understand what actually happens when an order is placed, on both
stock and crypto venues.

## Learn

- Order types (market/limit/stop), bid-ask spread, slippage
- Where stocks differ from crypto: trading hours, settlement, custody, fees,
  24/7 liquidity gaps
- Why backtests that ignore fees and slippage lie to you

## Read

- Chapters 1–3 of *Quantitative Trading* — Ernest Chan —
  [find it on Amazon](https://www.amazon.com/s?k=Quantitative+Trading+Ernest+Chan)

## Free tools

- [Alpaca API docs](https://docs.alpaca.markets/) (stocks) — read the docs
  even before writing code, they teach the mechanics
- [Binance API docs](https://binance-docs.github.io/apidocs/) (crypto)

## Steps

1. Read Chan chapters 1–3.
2. Read through the Alpaca and Binance API docs — order types, fee
   schedules, rate limits.
3. Write a short comparison table: stocks vs. crypto, on hours, settlement,
   fees, and liquidity.

## Milestone

- [ ] You can list three ways a live strategy underperforms its backtest,
      and why.

**Next:** [02-programming-data-pipeline.md](02-programming-data-pipeline.md)
